...
const updateTeam = async ({ id, ...teamData }) => {
  const response = await apiClient.put(`/admin/teams/${id}`, teamData);
  return response.data;
};

const addTeamMember = async ({ teamId, userId }) => {
  const response = await apiClient.post(`/admin/teams/${teamId}/members`, { userId });
  return response.data;
};

const removeTeamMember = async ({ teamId, userId }) => {
  const response = await apiClient.delete(`/admin/teams/${teamId}/members/${userId}`);
  return response.data;
};
...
const getAvailableTherapists = async () => {
  const response = await apiClient.get('/admin/users?role=therapist');
  return response.data.filter(user => 
    user.roles.some(role => role.name === 'therapist')
  );
};

const getAvailableBCBAs = async () => {
  const response = await apiClient.get('/admin/users?role=bcba');
  return response.data.filter(user => 
    user.roles.some(role => role.name === 'bcba')
  );
};
  ...
  return (
    <div className="h-full space-y-6">
              ...
              >
                ...
                {availableBCBAs?.map(bcba => (
                  <option key={bcba.id} value={bcba.id}>
                    {bcba.firstName} {bcba.lastName} ({bcba.email})
                  </option>
                ))}
              </select>
            </div>
      ...
      <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <h3 className="font-medium mb-2">Admin Team Management</h3>
        <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
          <li>• As an admin, you can manage all teams across the organization</li>
          <li>• Create teams and assign any BCBA as the team lead</li>
          <li>• Add or remove therapists from any team</li>
          <li>• Teams help organize therapists under specific BCBAs for better schedule management</li>
          <li>• Each therapist can only belong to one team at a time</li>
        </ul>
      </div>
    </div>
  );
